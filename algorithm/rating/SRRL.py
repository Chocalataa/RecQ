...
class Agent():
    def __init__(self, userNum, emb_dim, learning_rate=0.05):
        ...
        self.cross_entropy = -tf.multiply(self.mask,tf.log(tf.sigmoid(self.output)))-tf.multiply(self.negSample,tf.log(tf.sigmoid(self.neg)))
        ...
        self.pretrain = pre_opt.minimize(self.pretrain_loss,var_list=self.g_params)
